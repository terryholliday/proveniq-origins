...
export const memoirExportRoutes = Router();
...
interface NarrativeChapter {
  ...
  events: Array<{
    ...
    summary: string | null;
    ghostwrittenText?: string;
  }>;
}

// POST /api/memoir/generate - Generate AI-woven narrative from events
memoirExportRoutes.post('/generate', async (req: Request, res: Response) => {
  try {
    const { 
      chapterIds, 
      style = 'reflective',
    ...
    let unassignedEvents: typeof chapters[0]['events'] = [];
    if (includeAllEvents) {
      unassignedEvents = await prisma.event.findMany({
    ...
    const narrativeChapters: NarrativeChapter[] = [];
...
memoirExportRoutes.post('/legacy-letter', async (req: Request, res: Response) => {
  try {
    const { recipientName = 'my loved ones', themes = [] } = req.body;

    // Get keystone events and high-emotion events
    const keystoneEvents = await prisma.event.findMany({
...
memoirExportRoutes.post('/html', async (req: Request, res: Response) => {
  try {
...
<body>
  ...
  <div class="toc">
    <h2>Contents</h2>
    ${chapters.map((ch: NarrativeChapter, i: number) => `
      <div class="toc-entry">
  ...
  ${chapters.map((ch: NarrativeChapter) => `
    <div class="chapter">
...
memoirExportRoutes.get('/styles', (req: Request, res: Response) => {
  res.json([
