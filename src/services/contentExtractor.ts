...
async function analyzeImage(filePath: string): Promise<{ description: string; memoryPrompts: string[]; estimatedDate?: string }> {
  const openaiKey = process.env.OPENAI_API_KEY;
  if (!openaiKey) {
...
export async function extractContent(
  ...
  mimeType: string,
  artifactId?: string
): Promise<{ text: string; analysis?: string; memoryPrompts?: string[]; estimatedDate?: string }> {
  let text = '';
  let analysis = '';
  ...
  const absolutePath = path.isAbsolute(filePath) 
    ? filePath 
    : path.join(process.cwd(), 'uploads', filePath.replace(/^\/uploads\//, ''));
...
export async function processUnextractedArtifacts(): Promise<void> {
