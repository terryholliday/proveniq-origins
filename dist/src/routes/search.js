...
exports.searchRoutes.get('/', async (req, res) => {
    try {
...
exports.searchRoutes.post('/filter', async (req, res) => {
    try {
        const { text, searchIn, chapterIds, traumaCycleIds, personIds, tagIds, dateRange, isKeystone, hasArtifacts, hasSynchronicities, limit = 50, } = req.body;
        // Build where clause
        const where = {};
        // Text search
        if (text && typeof text === 'string' && text.length >= 2) {
            ...
            if (fields.includes('title'))
                textConditions.push({ title: { contains: text } });
            if (fields.includes('summary'))
                textConditions.push({ summary: { contains: text } });
            if (fields.includes('notes'))
                textConditions.push({ notes: { contains: text } });
